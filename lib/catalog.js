...
export async function loadPricingContext(client) {
  const [{ data: settings }, { data: marginDefaults }, { data: rates }] = await Promise.all([
    client.from('pricing_settings').select('*').single(),
    client.from('margin_defaults').select('*'),
 ...
 */
export async function loadCatalog(client, { haendler = null, includeInactive = false } = {}) {
  const [{ data: rows }, { data: prices }, ctx] = await Promise.all([
    client.from('products').select('*').order('sort_order'),
    client.from('product_prices').select('*'),
    loadPricingContext(client),
  ]);
